    ...
    for idx, pair in enumerate(eval_pairs):
        ...
        for _eval in data['evaluation_methods']:
            ...
            if 'ndv' == _eval['metric']:
                ...
                mask = nib.load(os.path.join(GT_PATH, pair['moving'])).get_fdata()[1:-1, 1:-1, 1:-1]
                mask = mask > 0
                if disp_field.shape[0] != 3:
...
def to_json(cases_results1, cases_results2, OUTPUT_PATH, name):
    cases_results = {**cases_results1, **cases_results2}
    aggregated_results = {}
    metrics = []
...
if __name__ == "__main__":
    ...
    cases_results1, name = evaluate_L2R(args.input_path, args.gt_path, args.output_path,
                 args.config_path1, args.verbose)
    cases_results2, name = evaluate_L2R(args.input_path, args.gt_path, args.output_path,
                 args.config_path2, args.verbose)
    to_json(cases_results1, cases_results2, args.output_path, name)
